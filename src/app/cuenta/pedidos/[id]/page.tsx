...
export default async function CuentaPedidoDetailPage({
  ...
  // Sesión del cliente + RLS: un pedido ajeno simplemente no existe aquí.
  // Solo campos del propio pedido (snapshot); nada técnico ni administrativo.
  const { data: order } = await supabase
    .from("orders")
    .select(
      "code, status, created_at, subtotal, delivery_fee, total, delivery_method, delivery_address, delivery_district, notes, order_items(id, item_name, quantity, unit_price, subtotal)"
    )
    .eq("id", id)
    .maybeSingle();
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-cacao">{formatDate(order.created_at)}</p>
      </div>
      ...
      <section className="rounded-3xl bg-blanco-crema p-6 shadow-calida">
        ...
        <p className="mt-3 text-sm text-cacao">
          Los precios corresponden al momento de tu compra.
        </p>
      </section>
      ...
      <section className="rounded-3xl bg-blanco-crema p-6 shadow-calida">
        ...
        <dl className="mt-3 space-y-2 text-tinta">
          <div>
            ...
            <dd>{order.delivery_method === "delivery" ? "Delivery" : "Recojo"}</dd>
          </div>
